...
QCustomPlot::QCustomPlot(QWidget *parent) :
  ...
  mReplotting(false),
  showStaffGauge(false),
  staffGauge(nullptr)
{
    staffGauge = new QPStaffGauge(this);
    qDebug() << mViewport;
...
void QCustomPlot::paintEvent(QPaintEvent *event)
{
  Q_UNUSED(event);
    ...
    pen.setColor(Qt::red);
    painter.setPen(pen);
    painter.drawLine(0,100,200,10);
//  QPainter painter(this);
  painter.drawPixmap(0, 0, mPaintBuffer);
  if(showStaffGauge)
  {
      painter.save();
      painter.setPen(staffGauge->linePen());
      painter.drawLine(axisRect()->left(), curPoint.y(), axisRect()->right(), curPoint.y());
      painter.restore();
  }
}

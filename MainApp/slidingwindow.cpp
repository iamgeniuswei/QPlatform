    ...
    , bPressFlag(false)
{
    ...
    contentFrame = new Form(splitter);
    contentFrame->setStyleSheet("background:red");
    contentFrame->resize(475, height());
    contentFrame->setMinimumWidth(405);
    listFrame = new QFrame(splitter);
    listFrame->setAttribute(Qt::WA_TranslucentBackground);
    listFrame->setStyleSheet("background:green");
